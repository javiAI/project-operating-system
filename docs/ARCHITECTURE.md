...
- **Informative** (SessionStart): safe-fail graceful — nunca emite `permissionDecision`, nunca exit 2. Payload malformado o errores de git degradan a `additionalContext` mínimo + log de error; exit 0 siempre. Bloquear un evento informativo dejaría al usuario sin contexto sin ganancia de enforcement. Referencia: `hooks/session-start.py` (D2).
...
- Exit 0 siempre. Payload malformado → snapshot mínimo `(error reading payload: ...)` + log a `.claude/logs/session-start.jsonl` + `phase-gates.jsonl` (evento `session_start`). Nunca emite `permissionDecision`.

...
- Failure mode (c.2) canonical: policy missing / malformed / sin sección `pre_compact` → log `status: policy_unavailable` + `additionalContext` mínimo `"pos pre-compact: policy unavailable (...)"`. Nunca deny blind — misma política canonizada por D5b para los tres hooks consumidores del loader.
- Safe-fail informative (excepción D2 reforzada): stdin vacío / JSON inválido / top-level no-dict / lista / escalar → exit 0 + `additionalContext` `"(error reading payload: ...)"` + log `status: payload_error`. Nunca `permissionDecision`, nunca exit 2.
...
- Fuente de invocaciones: `.claude/logs/skills.jsonl` (canonical audit log declarado en `policy.yaml.audit.required_logs`). Helper `_extract_invoked_skills(repo_root) → list[str]` ignora líneas corruptas, entries sin `skill`, `skill` no-string. Helper `_validate(invoked, allowed) → (decision, violations)` valida set-inclusion; deny si cualquier violation.
- Contrato diferenciado del accessor `skills_allowed_list`: **`None` = campo absent (deferred)** vs **`()` = lista declarada-vacía (deny-all explícito)**. Distinción obligatoria para expresar "allowlist vacía intencional = bloquear toda invocación" sin colisionar con "campo no declarado = enforcement off".
- Tres caminos de decisión real:
  ...
  3. `skills_allowed` declarado + invocaciones → `_validate(invoked, allowed)`. Violación → deny exit 2 con primer violador + guía literal `"Add it to the allowlist or revert the invocation."` en `decisionReason`. Sin violaciones → allow exit 0.
- Safe-fail blocker canonical (D1/D3/D4): stdin vacío / JSON inválido / top-level no-dict → deny exit 2 con `permissionDecision: deny` + `decisionReason` explicando la malformación. Un hook que no puede validar su input no debe dejarlo pasar (contrato reforzado desde D1).
- Double log **sólo en decisiones reales**: `stop-policy-check.jsonl` (allow/deny con `violations` list) + `phase-gates.jsonl` evento `stop`. Los status advisory (`deferred`, `policy_unavailable`) quedan sólo en el hook log — la puerta del lifecycle no se cruza hasta que la enforcement está realmente activa.
- Reuso `_lib/`: `policy.load_policy` + `policy.skills_allowed_list` + `append_jsonl` + `now_iso`. Sin nuevos helpers compartidos — los privados `_extract_invoked_skills` y `_validate` son específicos del dominio Stop (no se repiten en otros hooks) y viven dentro del propio archivo.

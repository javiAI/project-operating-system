...
- API: `load_policy(repo_root)` con cache keyed por path abs + mtime + size (invalidación implícita por edits al archivo; `reset_cache()` para tests) + 5 dataclasses congeladas (`ConditionalRule`, `DocsSyncRules`, `PostMergeTrigger`, `EnforcedPattern`, `PreWriteRules`) + 3 accessors tipados (`docs_sync_rules(repo_root)` / `post_merge_trigger(repo_root)` / `pre_write_rules(repo_root)`) + `derive_test_pair(rel_path, label)` con dos ramas label-driven (`hooks_top_level_py`, `generator_ts`).

...
- **Informative** (SessionStart): safe-fail graceful — nunca emite `permissionDecision`, nunca exit 2. Payload malformado degrada a `additionalContext` mínimo + log de error; errores de git/subprocess degradan a `additionalContext` mínimo **sin log** (absorbidos por `_git` como `None`). Fallos de logging (disk full, read-only fs) también se tragan. Exit 0 siempre. Bloquear un evento informativo dejaría al usuario sin contexto sin ganancia de enforcement. Referencia: `hooks/session-start.py` (D2).

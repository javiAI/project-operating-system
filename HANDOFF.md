...
- Fase actual: **D5 ✅ cerrada en rama** (`feat/d5-hook-post-action-compound`, PR pendiente de abrir). Anterior: **D4 ✅ PR #15** (`992137f`). Siguiente: **D6 — `feat/d6-hook-pre-compact-stop`**.
...
**D6 — `feat/d6-hook-pre-compact-stop`**
...
- [policy.yaml](policy.yaml) — secciones relevantes (`lifecycle.pre_compact`, `skills_allowed`). Aún sin parseador declarativo: D4 + D5 son las dos repeticiones hardcoded que abren la rama policy-loader; D6 puede ser el tercer hardcode o arrancar ya el parser — decisión en Fase -1.
- [.claude/rules/hooks.md](.claude/rules/hooks.md) — ahora con 5 hook references (D1..D5, 2 patrones canónicos: blocker + informative + PostToolUse non-blocking).
- [docs/ARCHITECTURE.md § 7 Determinismo](docs/ARCHITECTURE.md) — 5 hook blocks canónicos documentados.
- [hooks/pre-branch-gate.py](hooks/pre-branch-gate.py) + [hooks/pre-write-guard.py](hooks/pre-write-guard.py) + [hooks/pre-pr-gate.py](hooks/pre-pr-gate.py) como patrones de referencia blocker (3 aplicaciones); [hooks/post-action.py](hooks/post-action.py) como referencia PostToolUse non-blocking (D5). D6 vuelve al patrón blocker en PreCompact + Stop — shape D1 sirve como referencia.

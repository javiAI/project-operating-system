...
- El hook `pre-compact.py` **ya está vivo** desde D6: PreCompact **informative** (shape D2) — exit 0 siempre, nunca `permissionDecision`. Lee `pre_compact_rules(cwd).persist` del loader (D5b) y emite `additionalContext` con checklist de items a persistir antes del compact. Failure mode canónico (c.2): policy ausente o sección `lifecycle.pre_compact` ausente → loader devuelve `None` → hook emite contexto mínimo (`"(pre-compact policy unavailable — proceeding without checklist)"`) + log `status: policy_unavailable`. Safe-fail informative ante stdin corrupto: contexto degradado `"(error reading payload: ...)"` + log `status: payload_error`. Double log: `pre-compact.jsonl` siempre; `phase-gates.jsonl` evento `pre_compact` **solo** en happy path.
- El hook `stop-policy-check.py` **ya está vivo** desde D6 como **scaffold activable** — shape D1 blocker (safe-fail canónico deny en payload malformado), pero **sin enforcement en producción hoy**. Lee `skills_allowed_list(cwd)`: `None` → `status: deferred` + pass-through silencioso (sección `skills_allowed` ausente del `policy.yaml` del meta-repo); `()` → explicit deny-all; `tuple[str, ...]` poblada → enforcement live contra `.claude/logs/skills.jsonl`. La skill source es ese log canónico (lo pueblan las skills reales cuando Fase E* las entregue). Contrato `None` vs `()` deliberado: `None` señaliza "sección no declarada" (pass-through), `()` señaliza "declarada explícitamente vacía" (deny-all) — distinción semántica, no cosmética. Double log solo en decisiones reales (`deferred`/`policy_unavailable` van solo al hook log, no cruzan `phase-gates.jsonl`). Framing **anti-sobrerrepresentación**: hoy el hook NO protege nada en producción; la entrega D6 aporta el shape y la suite de tests que valida el contrato — el switch-on real llega cuando una skill poblada declare su allowlist.
...
`feat/d6-hook-pre-compact-stop` — sexto y séptimo hook Python, cierre de Fase D antes de arrancar Fase E (skills). Dos entregas en un PR deliberadamente (decisión Fase -1 "both-together"): ambos hooks comparten el mismo nuevo accessor pattern sobre el loader D5b (`pre_compact_rules` + `skills_allowed_list`), consumen un contrato `None`/`()` simétrico, y se testean contra el mismo fixture set — separarlos habría generado churn sin beneficio.

**`hooks/pre-compact.py` — PreCompact informative (shape D2, no-enforcement)**: lee `pre_compact_rules(cwd).persist` y emite checklist en `additionalContext` antes de que Claude Code haga `/compact`. Exit 0 siempre; nunca `permissionDecision`. Happy path: 5-line header + bullet list de items a persistir (patrones, decisiones, marker files). Failure mode canónico (c.2): policy ausente o `lifecycle.pre_compact` ausente → `build_context` minimal + log `status: policy_unavailable`. Safe-fail informative ante stdin corrupto: `"(error reading payload: ...)"` + log `status: payload_error` — el hook nunca rompe el compact, sólo degrada su contexto. Double log: `pre-compact.jsonl` siempre (incluye status y payload echo); `phase-gates.jsonl` evento `pre_compact` **solo** en happy path (policy_unavailable y payload_error no cruzan la puerta del lifecycle).

**`hooks/stop-policy-check.py` — Stop blocker scaffold (shape D1, NO enforcement en producción hoy)**: este es el punto de framing más importante de D6. Hoy el hook está **activable pero desactivado**: `policy.yaml` no declara `skills_allowed`, así que `skills_allowed_list(cwd)` devuelve `None` y el hook entra en camino `status: deferred` — exit 0, pass-through silencioso. Cuando E1a declare una skill real y pueble `skills_allowed`, el hook **ya está listo sin cambio de código**: lee `.claude/logs/skills.jsonl` via `_extract_invoked_skills`, compara contra la allowlist, y emite `permissionDecision: deny` + exit 2 si hay violación. Shape D1 canónico: safe-fail `_deny_payload` ante stdin malformado (deny canónico, nunca allow blind). Contrato `None` vs `()`: `None` = sección ausente (deferred) vs `()` = declarada explícitamente vacía (deny-all) — distinción semántica que sostiene el scaffold. Double log: solo en decisiones reales (deferred + policy_unavailable van al hook log, no cruzan `phase-gates.jsonl`).

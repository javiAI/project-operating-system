...
- El hook `pre-pr-gate.py` **ya está vivo** desde D4: PreToolUse(Bash) blocker sobre `gh pr create` únicamente. Bloquea con exit 2 cuando el diff `merge-base HEAD main → HEAD` no contiene los docs exigidos (baseline `ROADMAP.md` + `HANDOFF.md`; conditional por prefijo: `generator/**` | `hooks/**` excl. tests | `.claude/patterns/**` → `docs/ARCHITECTURE.md`; `skills/**` → `.claude/rules/skills-map.md`). Skip advisory (pass-through + log explícito en hook log, no en phase-gates) en `main` / `master` / HEAD detached / cwd no-git / main borrada localmente. Empty diff → deny dedicado con reason separado de docs-sync. 3 entradas advisory `deferred` (skills_required / ci_dry_run_required / invariants_check) se loguean en cada decisión real como scaffold activable sin cambio de shape cuando sus ramas dedicadas aporten sustrato. Reglas hardcoded (mirror de `policy.yaml.lifecycle.pre_pr.docs_sync_*`); migración a parser declarativo diferida a rama dedicada (junto con los paths hardcoded de D3).
...
`pre-pr-gate` vivo: en cada `PreToolUse(Bash)` detecta `gh pr create` (shlex + `tokens[:3] == ["gh","pr","create"]`). Skip advisory + log explícito en main/master/HEAD/git-unavailable/merge-base-unresolved. Empty diff → deny dedicado. Docs-sync check con reglas hardcoded (mirror de `policy.yaml.lifecycle.pre_pr.docs_sync_*`): baseline ROADMAP+HANDOFF + conditional por prefijo (`generator/**`, `hooks/**` excl. tests, `.claude/patterns/**` → `docs/ARCHITECTURE.md`; `skills/**` → `.claude/rules/skills-map.md`). Triggering paths capeados a 3 por doc con `... (+N more)`. 3 entradas advisory `deferred` (skills_required / ci_dry_run_required / invariants_check) emitidas sólo en decisiones reales. Safe-fail blocker canonical D1. Double log: `pre-pr-gate.jsonl` + `phase-gates.jsonl` (evento `pre_pr`). 96 tests D4, 317 totales en `hooks/**`, 93% coverage sobre `pre-pr-gate.py`; D1/D2/D3 intactos.

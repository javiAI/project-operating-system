...
- `bin/tests/test_template_loader_contract.py` (NEW, 182 líneas) — fixture `module-scoped` parametrizada sobre los 3 profiles canónicos (cli-tool, nextjs-app, agent-sdk) que genera el proyecto sintético una vez por profile. 5 clases × 9 tests × 3 profiles = 27 cases parametrizados: `TestPreWriteRules` (returns dataclass + empty tuple), `TestDocsSyncRules` (returns dataclass + ROADMAP/HANDOFF en baseline), `TestPostMergeTrigger` (returns dataclass + min_files_changed int no bool), `TestPreCompactRules` (returns dataclass + 3 items canónicos), `TestSkillsAllowed` (returns None per A2).

...
- 5 accessors: `docs_sync_rules(repo_root)` / `post_merge_trigger(repo_root)` / `pre_write_rules(repo_root)` / `pre_compact_rules(repo_root)` / `skills_allowed_list(repo_root)`. Cada uno encapsula un lookup de sección concreto en `policy.yaml` + construcción de la dataclass (o `tuple[str, ...] | None` para `skills_allowed_list`, que no justifica dataclass wrapper para un solo campo). **Contrato diferenciado de `skills_allowed_list` (D6, c.3 scaffold)**: `None` = campo absent (deferred — el consumer pasa); `()` = lista declarada-vacía (deny-all explícito — el consumer enforza). Esta distinción permite que el hook Stop (séptimo) separe "sin allowlist declarada" (prod hoy, pass-through) de "allowlist vacía intencional" (bloquea cualquier invocación). Si un hook futuro consume una sección nueva, añadir un accessor siguiendo el mismo patrón — NO reparsear `policy.yaml` dentro del hook.
...
- Contrato de tres caminos de decisión real:
  1. `policy.yaml` missing/malformed → log `status: policy_unavailable`, pass-through exit 0 (cero stdout). Mismo shape que los otros hooks post-D5b.
  2. `policy.yaml` OK sin `skills_allowed` → log `status: deferred`, pass-through exit 0. **Estado actual de prod**.
  3. `skills_allowed` declarado + invocaciones → deny exit 2 con primer violador + guía literal `"Add it to the allowlist or revert the invocation."` en `decisionReason`; sin violaciones → allow exit 0.
- Fuente de invocaciones: `.claude/logs/skills.jsonl` — canonical audit log declarado en `policy.yaml.audit.required_logs`. Helper `_extract_invoked_skills(repo_root) → list[str]` ignora silenciosamente: archivo ausente, líneas non-JSON, entries sin `skill`, entries con `skill` no-string. El hook es enforcer, no forense.
...
- Safe-fail blocker canonical (D1/D3/D4): stdin vacío / JSON inválido / top-level no-dict → deny exit 2 con `permissionDecision: deny` + `decisionReason` explicando la malformación. El hook no puede dejar pasar lo que no puede validar.
- Double log **sólo en decisiones reales**: `stop-policy-check.jsonl` (`{ts, hook, decision, violations, reason}` en deny; `{ts, hook, decision: allow, invoked_count}` en allow) + `phase-gates.jsonl` evento `stop` (con `decision: allow|deny`). Los status advisory (`deferred`, `policy_unavailable`) quedan sólo en el hook log — la puerta del lifecycle no se cruza hasta que la enforcement está realmente activa.
- Reuso `_lib/`: `policy.load_policy` + `policy.skills_allowed_list` + `append_jsonl` + `now_iso`. `_extract_invoked_skills` y `_validate` son privados específicos del dominio Stop — no se repiten en otros hooks, no migran a `_lib/` (regla #7: ≥2 consumidores antes de extraer).
- Tests: 35 casos en `hooks/tests/test_stop_policy_check.py` (8 clases: `TestOutputEnvelope`, `TestDeferredMode`, `TestActivableEnforcement`, `TestSafeFailBlocker`, `TestLogging`, `TestMainInProcess`, `TestExtractInvokedSkillsUnit`, `TestValidateSkillsUnit`). Fixture `stop.json`. Helper `write_skills_allowed(repo, allowed)` escribe `policy.yaml` mínimo para activar enforcement; `seed_skills_log(repo, skills)` escribe `.claude/logs/skills.jsonl` para simular invocaciones. El module docstring del test file reitera explícitamente el framing anti-sobrerrepresentación.

Suite global `hooks/**` tras D6: **555 passed + 1 skipped** (D1+D2+D3+D4+D5+D5b+D6), sin regresión.

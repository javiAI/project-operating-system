...
## Renderers (entregado en C1, ampliado en C2 y C3)
...
- **`FileWrite` shape**: `{ path: string; content: string }`. `path` relativo al root del repo generado (subdirs permitidos). Sin `mode` en C1/C2/C3 — se añade en C5 si hooks ejecutables lo requieren.
...
- **Composición en [generator/renderers/index.ts](../../generator/renderers/index.ts)** (pattern `renderer-group`, 3ª aplicación consolidada en C3): arrays congelados por grupo — `coreDocRenderers` (6 renderers, C1), `policyAndRulesRenderers` (2 renderers, C2), `testsHarnessRenderers` (1 renderer, C3). `allRenderers = Object.freeze([...coreDocRenderers, ...policyAndRulesRenderers, ...testsHarnessRenderers])`. `generator/run.ts` importa **sólo** `allRenderers` — la composición no vive en `run.ts`. Añadir un nuevo grupo (C4, C5) = crear array congelado nuevo y concatenarlo dentro de `allRenderers`, no tocar `run.ts`.
...
- **Snapshots**: `generator/__snapshots__/<profile-slug>/<path>.snap`. C1 = 18 (3 × 6); C2 = 27 (3 × 9); C3 = 39 (+12: 2 profiles TS × 4 + 1 profile Python × 4). Desde C3 el set por profile varía por stack, así que el total ya no es estrictamente `profiles × templates`.
...
- **`--force` flag** — fuera de scope C1/C2/C3. `--out` sobre dir no vacío aborta con exit 3 protegiendo output del usuario. Reabrir cuando un flujo legítimo lo exija.
- **Rules path-scoped adicionales (`generator.md`, `templates.md`, `tests.md`, `ci-cd.md`, `skills-map.md`)** — fuera de scope C2/C3. Se añadirán en ramas posteriores sólo cuando aparezca señal de stack-specificidad real o de duplicación entre el meta-repo y el proyecto generado.
...
- **Materialización de defaults del schema en `buildProfile`** (C3) — diferida. Workaround actual: declarar explícitamente en cada profile los paths con `default:` que los templates referencian (ver fixture `valid-partial`). Reabrir como rama propia si C4/C5 revelan más paths afectados.

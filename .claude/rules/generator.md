...
- **CI/CD (C4)**: `ci-cd.ts` (renderer único, hasta 2 archivos por profile). Soportado: `workflow.ci_host == "github"` → `.github/workflows/ci.yml` siempre + `docs/BRANCH_PROTECTION.md` si `workflow.branch_protection == true`. `ci_host ∈ {gitlab, bitbucket}` → `Error` explícito desde el renderer con host + "deferred" + path `workflow.ci_host` (mismo patrón que frameworks diferidos de C3). `ci.yml` es estable (`name: ci`, job `unit`, stack conditionals Node 20.17.0 / Python 3.11) e **invoca `make test-unit` + `make test-coverage`** exclusivamente (entry-point universal delegado al Makefile de C3, nunca `npx vitest`/`pytest` directos). Cada stack declara un step `Install test deps` con versiones pinneadas (TS: `npm install --no-save vitest@3.0.5 @vitest/coverage-v8@3.0.5`; Python: `pip install pytest pytest-cov`) para cerrar el contrato de ejecutabilidad del workflow cuando C3 no emite `package.json` / `pyproject.toml`. Tests semánticos validan presencia del step + versiones pinneadas en TS, y no-leak en Python (sin `npm`/`vitest`). Todos los `uses:` pinneados por SHA40 (`.claude/rules/ci-cd.md`). `${{ github.* }}` literales se escapan con `\{{` en el template Handlebars. `BRANCH_PROTECTION.md` es markdown dinámico para aplicación manual en GitHub Settings (el generador no llama la API de GitHub; separación control-plane vs runtime-plane); el comentario de cabecera del template deja explícito que la lista de required checks se actualiza a mano.
...
- **Pinning de `vitest` / `@vitest/coverage-v8` dentro del workflow (C4, contrato provisional)** — C4 pinnea las versiones directamente en `ci.yml` (alineadas con `package.json` del meta-repo) para cerrar el contrato de ejecutabilidad sin esperar a `package.json` emitido. Cuando C5/C6 emita `package.json` en el proyecto generado, reemplazar el step por `npm ci` y mover los pins al manifest. Mismo esquema aplicará a `pytest`/`pytest-cov` cuando se emita `pyproject.toml`.

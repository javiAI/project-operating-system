...
└── renderers/
    ├── index.ts            # coreDocRenderers + policyAndRulesRenderers + testsHarnessRenderers + cicdRenderers + skillsHooksRenderers + allRenderers (congeladas)
    ├── claude-md.ts        # C1
    ├── master-plan.ts      # C1
    ├── roadmap.ts          # C1
    ├── handoff.ts          # C1
    ...
    └── skills-hooks-skeleton.ts  # C5 — .claude/settings.json + .claude/hooks/README.md + .claude/skills/README.md (copia real hooks+skills diferida)
```
...
- **`FileWrite` shape**: `{ path: string; content: string }`. `path` relativo al root del repo generado (subdirs permitidos). Sin `mode` en C1–C5 — se añade en la primera rama que copie ejecutables reales (post-D1/E1a). C5 emite sólo esqueleto `.claude/`, no binarios; la extensión sigue sin justificar.
...
- **Composición en [generator/renderers/index.ts](../../generator/renderers/index.ts)** (pattern `renderer-group`, 5ª aplicación consolidada en C5): arrays congelados por grupo — `coreDocRenderers` (6 renderers, C1), `policyAndRulesRenderers` (2 renderers, C2), `testsHarnessRenderers` (1 renderer, C3), `cicdRenderers` (1 renderer, C4), `skillsHooksRenderers` (1 renderer, C5). `allRenderers = Object.freeze([...coreDocRenderers, ...policyAndRulesRenderers, ...testsHarnessRenderers, ...cicdRenderers, ...skillsHooksRenderers])`. `generator/run.ts` importa **sólo** `allRenderers` — la composición no vive en `run.ts`. Añadir un nuevo grupo = crear array congelado nuevo y concatenarlo dentro de `allRenderers`, no tocar `run.ts`.
...
- **Skills + hooks skeleton (C5)**: `skills-hooks-skeleton.ts` (renderer único, 3 archivos por profile, independientes del stack). Emite `.claude/settings.json` (JSON estático: `hooks: {}` + `_note` con texto explicando la deferral a Fase D; **no** siembra `permissions` baseline — decisión explícita del usuario), `.claude/hooks/README.md` (documenta deferral a Fase D + referencia a la extensión futura de `FileWrite.mode`) y `.claude/skills/README.md` (documenta deferral a Fase E + catálogo auditado). Cierra la *estructura* del directorio `.claude/` del proyecto generado pero **no** copia hooks ejecutables ni skills — ambos diferidos a rama post-D1/E1a. Tests validan: JSON parseable, `hooks === {}`, `_note` string >40 chars con `/pos/`, READMEs matching `/\bpos\b/` + `/Fase\s*D|E/` + `/diferid/i`, stack-agnostic (sin leaks `vitest`/`pytest`/`npm`/`pip`), trailing `\n`, determinismo byte-identical.
- **CI/CD (C4)**: `ci-cd.ts` (renderer único, hasta 2 archivos por profile). Soportado: `workflow.ci_host == "github"` → `.github/workflows/ci.yml` siempre + `docs/BRANCH_PROTECTION.md` si `workflow.branch_protection == true`. `ci_host ∈ {gitlab, bitbucket}` → `Error` explícito desde el renderer con host + "deferred" + path `workflow.ci_host` (mismo patrón que frameworks diferidos de C3). `ci.yml` es estable (`name: ci`, job `unit`, stack conditionals Node 20.17.0 / Python 3.11) e **invoca `make test-unit` + `make test-coverage`** exclusivamente (entry-point universal delegado al Makefile de C3, nunca `npx vitest`/`pytest` directos). Cada stack declara un step `Install test deps` con versiones pinneadas (TS: `npm install --no-save vitest@3.0.5 @vitest/coverage-v8@3.0.5`; Python: `pip install pytest==8.3.4 pytest-cov==6.0.0`) para cerrar el contrato de ejecutabilidad del workflow cuando C3 no emite `package.json` / `pyproject.toml`. Tests semánticos validan en ambas ramas presencia del step + versiones pinneadas + orden pre-`make test-unit`, más no-leak cruzado (TS sin `pip`/`pytest`; Python sin `npm`/`vitest`). Todos los `uses:` pinneados por SHA40 (`.claude/rules/ci-cd.md`). `${{ github.* }}` literales se escapan con `\{{` en el template Handlebars. `BRANCH_PROTECTION.md` es markdown dinámico para aplicación manual en GitHub Settings (el generador no llama la API de GitHub; separación control-plane vs runtime-plane); el comentario de cabecera del template deja explícito que la lista de required checks se actualiza a mano.
...
- **Snapshots**: `generator/__snapshots__/<profile-slug>/<path>.snap`. C1 = 18 (3 × 6); C2 = 27 (3 × 9); C3 = 39 (+12: 2 profiles TS × 4 + 1 profile Python × 4); C4 = 45 (+6: 3 profiles × 2 archivos — `.github/workflows/ci.yml.snap` + `docs/BRANCH_PROTECTION.md.snap`); C5 = 54 (+9: 3 profiles × 3 archivos — `.claude/settings.json.snap` + `.claude/hooks/README.md.snap` + `.claude/skills/README.md.snap`). Desde C3 el set por profile varía por stack y por flags, así que el total ya no es estrictamente `profiles × templates`.
...
- **Pinning de deps de test dentro del workflow (C4, contrato provisional)** — C4 pinnea las versiones directamente en `ci.yml` en ambos stacks (TS: `vitest@3.0.5` + `@vitest/coverage-v8@3.0.5` alineados con `package.json` del meta-repo; Python: `pytest==8.3.4` + `pytest-cov==6.0.0`) para cerrar el contrato de ejecutabilidad sin esperar a `package.json` / `pyproject.toml` emitidos. Cuando C5/C6 emita esos manifests en el proyecto generado, reemplazar el step por `npm ci` / `pip install -e .[dev]` y mover los pins al manifest correspondiente.
- **Copia real de hooks ejecutables en el proyecto generado** (C5) — diferida. Motivo: el directorio `hooks/` del meta-repo no existe todavía (Fase D lo poblará); copiar placeholders o binarios inertes sería abstracción prematura (CLAUDE.md regla #7). C5 sólo emite `.claude/hooks/README.md` documentando el gap. Reabrir en rama post-D1 cuando exista el catálogo de hooks Python versionado.
- **Copia real de skills en el proyecto generado** (C5) — diferida. Motivo: las skills del plugin viven en un catálogo en evolución activa; una instantánea copiada hoy garantiza drift inmediato. C5 sólo emite `.claude/skills/README.md` documentando el gap. Reabrir en rama post-E1a cuando el catálogo esté auditado (`/pos:audit-plugin --self` green).
- **`FileWrite.mode` — extensión del shape del generador** (C5) — diferida. Motivo: mientras `pos` no copie ejecutables reales al proyecto generado, `{ path, content }` basta. C5 no emite binarios (solo JSON + 2 READMEs), por lo que `mode` sigue sin justificar. Reabrir en la misma rama que primero copie ejecutables reales (probablemente post-D1/E1a): añadir `mode?: number` al tipo `FileWrite`, extender `writeFiles` para respetarlo con `fs.chmod`, y documentar la convención en `.claude/rules/generator.md § Renderers`. Snapshots existentes no se invalidan porque el shape sigue siendo compatible (`mode` opcional).
- **`permissions` baseline en `.claude/settings.json`** (C5) — diferida. Motivo: decisión explícita del usuario — mínimo conservador. C5 emite `hooks: {}` + `_note` sin declarar `permissions` para no sembrar una superficie más amplia de lo necesario. Cuando Fase D introduzca hooks reales con superficie de permisos conocida, decidir si el renderer materializa un baseline mínimo o si la responsabilidad queda en el hook mismo al instalarse.

## Reuso desde `tools/lib/` (pattern-before-abstraction, 5ª aplicación renderer-group)
...
5. **5ª (C5)** — `skillsHooksRenderers` consolida el pattern `renderer-group` por 5ª vez (core + policy/rules + tests-harness + ci-cd + skills-hooks-skeleton). Cada grupo sigue el mismo shape: frozen tuple exportado desde `generator/renderers/index.ts` + composición en `allRenderers`. `run.ts` nunca se toca al añadir un grupo nuevo. No justifica extraer `lib/renderer-group.ts` — la composición es expresada íntegramente en `Array.prototype.concat` + `Object.freeze`; bifurcar sólo si aparece lógica específica (p.ej. precondiciones inter-grupo).

Si aparece una aplicación que exija lógica específica (p.ej. loader que además valide estructura del template, o composición inter-grupo con precondiciones), bifurcar entonces.
